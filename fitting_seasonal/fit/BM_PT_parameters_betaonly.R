...
all_data_ages <- data.table(read.csv("AWS/all_data_ages.csv"),
                            stringsAsFactors =F)
...
#all_data_ages_m <- all_data_ages_m[year_week >= as.Date("2015-07-01")]
...
seasonal_15_20$value <- round(seasonal_15_20$value *0.54)
seasonal_19_20$value <- round(seasonal_19_20$value *0.54)

# seasonal_15_20$type <- "beta only"
# seasonal_15_20_full$type <- "all subgroups"
# 
# to_plot <- rbind(seasonal_15_20, seasonal_15_20_full)

# BETAONLY <- ggplot(to_plot, aes(x = year_week, y = value, colour = type, group = type)) + 
#   geom_point() + 
#   geom_path() +
#   facet_grid(Age~.) + 
#   theme_linedraw() + 
...
#   labs(x = "Week", y = "Cases", colour = "Type")
# 
# tiff(here("figures","beta_only_data.tiff"), height = 2000, width = 3600, res = 300)
# BETAONLY
# dev.off()
